...
export default function CartSidebar({ isOpen, onClose }) {
  ...
  const activeOrders = orders.filter(order => {
    const isActive = !["entregado", "archivado"].includes(order.status) && !order.archivado;
    const isGuest = order.user === "invitado";
    if (user) {
      return order.usuarioId === user.uid && isActive;
    }
      ...
      <aside className={`cart-sidebar ${isOpen ? "cart-sidebar--open" : ""}`}>
        <div className="cart-header">
          <h2 className="cart-title">
            <img src={logoFarmaGen} alt="" className="cart-title-logo" />
            Farmacia FarmaGen
          </h2>
            ...
            activeOrders.length === 0 ? (
              <div className="cart-empty" style={{ justifyContent: "center", height: "100%" }}>
                ...
                <p style={{ fontSize: "0.8rem", textAlign: "center", color: "#94a3b8", padding: "0 20px" }}>
                  Cuando realices un pedido, podras seguir su preparacion desde aqui.
                </p>
              </div>
            ) : (
                  ...
                  <div key={order.id} style={{
                    ...
                    <div style={{
                      ...
                      <span style={{ fontSize: "0.75rem", fontWeight: "600", color: "#064e3b" }}>
                        Pedido registrado para recoger en sucursal.
                      </span>
                    </div>
                    ...
                    <div style={{ fontSize: "0.8rem", color: "var(--text-muted)", display: "flex", flexDirection: "column", gap: "4px" }}>
                      ...
                      <div><strong>Entrega:</strong> Recoger en Sucursal</div>
                      {branch && (
                        <div style={{
                          marginTop: "6px",
                          padding: "8px",
